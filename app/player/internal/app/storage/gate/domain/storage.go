...
func (do *StorageDomain) AfterUpdate(ctx core.Context, updateInfo *object.UpdateInfo, src adv1.ItemSource, silent bool) {
	...
	_ = ctx.EmitEvent(core.EventStorageItemUpdated, life.With(core.ArgKeyStorageItemIDs, itemIds))
	_ = ctx.EmitEvent(core.EventStoragePackUpdated, life.With(core.ArgKeyStoragePackIDs, packIds))

	ctx.Changed(object.ModuleKey)

	if !silent {
		_ = ctx.Push(int32(climod.ModuleID_Storage), int32(cliseq.StorageSeq_PushItemUpdated), ctx.UID(), amountMsg)
	}
}
