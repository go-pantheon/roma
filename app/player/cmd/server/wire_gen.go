...
func initApp(confServer *conf.Server, label *conf.Label, recharge *conf.Recharge, confRegistry *conf.Registry, confData *conf.Data, logger log.Logger, healthServer *health.Server) (*kratos.App, func(), error) {
	...
	gateRouteTable := broadcaster.NewGateRouteTable(universalClient)
	...
	conn, err := broadcaster.NewConn(logger, gateRouteTable, discovery)
	if err != nil {
		cleanup2()
		cleanup()
		return nil, nil, err
	}
	publisher := broadcaster.NewPublisher(conn)
	pushRepo := data3.NewPushRepo(publisher, logger)
	...
	clearCommander := storage.NewClearCommander(devUseCase, storageDomain)
	...
	registrar := cmdregistrar.NewRegistrar(showTimeCommander, changeTimeCommander, addItemCommander, subItemCommander, addPackCommander, subPackCommander, clearCommander, simulateRechargeCommander, createAdminPlayerCommander)
